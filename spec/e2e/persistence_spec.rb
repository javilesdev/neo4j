describe Neo4j::ActiveNode do
  ...
  describe '#update_node_propert(y|ies)' do
    before do
    ...
    shared_examples_for 'an update using update_node_propert(y|ies)' do
    ...
    describe 'singular `update_node_property`' do
      let(:update!) { person.update_node_property(:age, 20) }
      it_behaves_like 'an update using update_node_propert(y|ies)'

      it 'rejects undeclared values' do
        expect { person.update_node_property(:foo_col, 'foo') }.to raise_error NoMethodError
      end

      it 'performs type conversion' do
        expect do
          person.update_node_property(:age, '20')
      ...
      context 'on a new record' do
        it do
          expect do
            UpdateWithValidations.new.update_node_property(:age, 20)
          end.to raise_error(Neo4j::Error, /can not update on a new record object/)
        end
      end
    end

    describe 'plurual `update_node_properties`' do
      let(:update!) { person.update_node_properties(age: 20) }
      it_behaves_like 'an update using update_node_propert(y|ies)'

      it 'rejects undeclared values' do
        expect { person.update_node_properties(foo_col: 'foo') }.to raise_error NoMethodError
      end

      it 'performs type conversion' do
        expect do
          person.update_node_properties(age: '20')
        end.not_to change { UpdateWithValidations.find(person).age.is_a?(Numeric) }.from(true)
      end
      ...
      context 'on a new record' do
        it do
          expect do
            UpdateWithValidations.new.update_node_properties(age: 20)
      ...
      context 'when an error occurs in the body of the error' do
        ...
        it 'rolls back the database change' do
          expect do
            begin
              person.update_node_properties(foo_val: foo_val)
